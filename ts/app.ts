import { TPhong, TBuffet, THoBoi, TPhongGym} from './data.js'
...
export const showlistphong = () => {
     const html_arr = phong_arr.map(hien1Phong); //call back
     return html_arr.join("");
}

const hien1Buffet = (buffet:TBuffet) => {
    return `<div>
...
export const showlistbuffet = () => {
    const html_arr = buffet_arr.map(hien1Buffet); //call back
    return html_arr.join("");
}

const hien1HoBoi = (hoboi:THoBoi) =>{
    return `<div>
...
const hien1PhongGym = (gym:TPhongGym) =>{
    return `<div>
     <h4>Gym: ${gym.ten} </h4>
